...
class AvodModel(model.DetectionModel):
    ...
    def build(self):
        ...
        with tf.variable_scope('avod_roi_pooling'):
            ...
            bev_rois = tf.image.crop_and_resize(
                ...
                bev_proposal_boxes_norm_tf_order,
                tf_box_indices,
                self._proposal_roi_crop_size, #WZN: 7 for peds
                name='bev_rois')
        ...
        #WZN: The prediction already contains all from the rpn_model, now add others from avod_model
    ...
    #WZN: only have loss of 3D boxes
