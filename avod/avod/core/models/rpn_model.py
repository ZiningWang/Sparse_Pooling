...
class RpnModel(model.DetectionModel):
    ...
    def __init__(self, model_config, train_val_test, dataset):
        ...
        if hasattr(self._config.rpn_config,'rpn_use_sparse_pooling'):
            if dataset.config.output_indices:
                if self._config.rpn_config.rpn_use_sparse_pooling:
                    print('WZN: Using Non-homogeneous Sparse Pooling Layer. \n')
            else:
                self._config.rpn_config.rpn_use_sparse_pooling = False
        else:
            self._config.rpn_config.rpn_use_sparse_pooling = False
    ...
    def _set_up_input_pls(self):
        ...
        with tf.variable_scope('pl_sparse_pooling'):
            ...
            self._add_placeholder(tf.int32, [None, 3],
                                  self.PL_BEV_POOL_IJ)
        #WZN: another sparse pooling layer
        with tf.variable_scope('pl_sparse_pooling_after_vgg'):
            self._add_placeholder(tf.int64, [None, 2],
                                  self.PL_M_IJ_VGG)
            self._add_placeholder(tf.float32, [None],
                                  self.PL_M_VAL_VGG)
            self._add_placeholder(tf.int64, [2],
                                  self.PL_M_SIZE_VGG)
            self._add_placeholder(tf.int32, [None, 3],
                                  self.PL_IMG_POOL_IJ_VGG)
            self._add_placeholder(tf.int32, [None, 3],
                                  self.PL_BEV_POOL_IJ_VGG)
    ...
    def _set_up_feature_extractors(self):
        ...
        if self._config.rpn_config.rpn_sparse_pooling_after_vgg:
            ...
            self.bev_feature_maps, self.img_feature_maps, self.bev_end_points, self.img_end_points = \
                self._fusion_feature_extractor.build(
                    ...
                    self._is_training)
            print('WZN: Successfully fused feature after vgg and before fpn \n')
        ...
        if self._config.rpn_config.rpn_use_sparse_pooling:
            use_bn = self._config.rpn_config.rpn_sparse_pooling_use_batch_norm
            ...
            if self._config.rpn_config.rpn_sparse_pooling_conv_after_fusion:
        ...
        #WZN: Here is where he shrinks the channels of the features.
        #This is also what is expected to be removed after introducing NHPL
    ...
    def create_feed_dict(self, sample_index=None):
        ...
        if self._config.rpn_config.rpn_sparse_pooling_after_vgg:
            sparse_pooling_input2 = sparse_pooling_inputs[1]
            self._placeholder_inputs[self.PL_M_VAL_VGG] = sparse_pooling_input2['M_val']
            self._placeholder_inputs[self.PL_M_IJ_VGG] = sparse_pooling_input2['Mij_pool']
            self._placeholder_inputs[self.PL_M_SIZE_VGG] = sparse_pooling_input2['M_size']
            self._placeholder_inputs[self.PL_IMG_POOL_IJ_VGG] = sparse_pooling_input2['img_index_flip_pool']
            self._placeholder_inputs[self.PL_BEV_POOL_IJ_VGG] = sparse_pooling_input2['bev_index_flip_pool']
        else:
            self._placeholder_inputs[self.PL_M_VAL_VGG] = np.zeros((0))
            self._placeholder_inputs[self.PL_M_IJ_VGG] = np.zeros((0,2))
            self._placeholder_inputs[self.PL_M_SIZE_VGG] = np.zeros((2))
            self._placeholder_inputs[self.PL_IMG_POOL_IJ_VGG] = np.zeros((0,3))
            self._placeholder_inputs[self.PL_BEV_POOL_IJ_VGG] = np.zeros((0,3))
