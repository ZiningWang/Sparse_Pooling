...
def bbox_transform(ex_rois, gt_rois):
    #WZN: [dx,dy,dw,dh] from first [x0,y0,x1,y1] to second
...
def bbox_transform_3d(ex_rois_3d, gt_rois_3d):
    #WZN: 3D from first [x, y, z, l, w, h] to second
    # x, y, z, l, w, h
...
#def bbox_transform_cnr_2d(ex_center_2d, gt_rois_2d):

#    #
#    targets = gt_rois_2d[:,0:8].copy()
#    targets[:,0:4] -= ex_center_2d[:,0]
#    targets[:,4:8] -= ex_center_2d[:,1]

#    return targets

def bbox_transform_cnr_3d(ex_center_3d, gt_rois_3d):

    targets = gt_rois_3d[:,0:24].copy()
    # box diagonal distance
    targets[:,0:8] -= ex_center_3d[:,0:1]
    targets[:,8:16] -= ex_center_3d[:,1:2]
    targets[:,16:24] -= ex_center_3d[:,2:3]

    return targets

#WZN: the original one
...
#WZN: directly add to center
def bbox_transform_inv_cnr(centers, deltas):
    if centers.shape[0] == 0:
        return np.zeros((0, deltas.shape[1]), dtype=deltas.dtype)
    assert deltas.shape[1]==24

    pred_corners = np.copy(deltas)

    pred_corners[:,0:8] += centers[:,0:1]
    pred_corners[:,8:16] += centers[:,1:2]
    #pred_corners[:,16:24]

    return pred_corners

'''
WZN: original, based on anchor box
