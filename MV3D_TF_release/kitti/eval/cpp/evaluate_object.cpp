...
tPrData computeStatistics(CLASSES current_class, const vector<tGroundtruth> &gt,
  ...
  for(int32_t i=0; i<gt.size(); i++){
    ...
    for(int32_t j=0; j<det.size(); j++){
      ...
      //printf("IoU: %.3f, thresholds: %.3f\n", overlap,MIN_OVERLAP[metric][current_class]);
    ...
    //printf("%d, %d\n",ignored_gt[i],ignored_det[det_idx]);
  ...
  if(compute_fp){
    ...
    for(int32_t i=0; i<det.size(); i++){
      ...
      //printf("%d, %d, %d, %d\n",int(assigned_detection[i]),ignored_det[i],ignored_det[i],int(ignored_threshold[i]));
...
bool eval_class (FILE *fp_det, FILE *fp_ori, CLASSES current_class,
  ...
  int ngt = groundtruth.size();
  int ndet = detections.size();
  printf("num of groundtruth: %d, num of detections: %d \n",ngt,ndet);
...
bool eval(string gt_dir, string result_dir, string data_list_path){
 //mail = new Mail();
  ...
  printf("Loading detections...%d images.\n",N_VALIMAGES);
  for (int32_t i=0; i<N_VALIMAGES; i++) {
    ...
    // check for errors
    if (!gt_success) {
      printf("ERROR: Couldn't read: %s of ground truth. Please write me an email!", file_name.c_str());
      return false;
    }
    if (!det_success) {
      printf("ERROR: Couldn't read: %s", file_name.c_str());
      return false;
    }
  }
  printf("  done.");
